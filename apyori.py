...
class TransactionManager(object):
    ...
    def calc_support(self, items):
        ...
        #print(len(sum_indexes), float(len(sum_indexes)) / self.__num_transaction, )
...
def gen_support_records(transaction_manager, min_support, **kwargs):
    ...
    while candidates:
        ...
        for relation_candidate in candidates:
            ...
            #if length==1:
                #print(length,candidate_set, support)
            yield SupportRecord(candidate_set, support)
        #print('here')
        #print('relations',relations)
        print(len(relations),length)
        length += 1
...
def gen_ordered_statistics(transaction_manager, record):
    ...
    for combination_set in combinations(sorted(items), len(items) - 1):
        ...
        if len(items_add)==1 and len(items_base)>0:
            confidence = (
                record.support / transaction_manager.calc_support(items_base))
            lift = confidence / transaction_manager.calc_support(items_add)
            yield OrderedStatistic(
                frozenset(items_base), frozenset(items_add), confidence, lift)
            #print('items',items,'combination_set',combination_set,confidence,lift,record.support,transaction_manager.calc_support(items_base), transaction_manager.calc_support(items_add)  )
...
def apriori(transactions, **kwargs):
    ...
    print(min_support,min_confidence,min_lift,max_length)
    if min_support <= 0:
    ...
    #print('supportrecord',len(support_records))
    print('helo')
    # Calculate ordered stats.
    m=0
    for support_record in support_records:
        m+=1
       # print('support_record',support_record)
